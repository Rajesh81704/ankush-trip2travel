...
import { Preset, PresetType, PresetHotelData, PresetSightseeingData, PresetItineraryData, PresetFlightData, PresetImage } from "@/types/preset";
...
import {
  Plus, Trash2, Hotel, Camera, Clock, MapPin, Star, Plane,
...
export default function PresetsPage() {
  ...
  const [flightImages, setFlightImages] = useState<PresetImage[]>([]);
  ...
  const openEditModal = (preset: Preset) => {
    ...
    } else if (preset.type === "flight") {
      const f = preset.data as PresetFlightData;
      setFlightAirline(f.airline || "");
      setFlightNumber(f.flightNumber || "");
      setFlightDepartureCity(f.departureCity || "");
      setFlightArrivalCity(f.arrivalCity || "");
      setFlightDepartureTime(f.departureTime || "");
      setFlightArrivalTime(f.arrivalTime || "");
      setFlightClass(f.class || "economy");
      setFlightPrice(f.price || 0);
      setFlightDescription(f.description || "");
      setFlightImages(f.images || []);
    }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, target: "hotel" | "sightseeing" | "flight") => {
    const files = Array.from(e.target.files || []);
    if (files.length === 0) return;

    setUploading(true);
    try {
      const folder = target === "hotel" ? "hotels" : target === "sightseeing" ? "sightseeings" : "flights";
      ...
      } else if (target === "sightseeing") {
        setSightImages((prev) => [...prev, ...formatted]);
      } else if (target === "flight") {
        setFlightImages((prev) => [...prev, ...formatted]);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formType === "hotel") {
      ...
      if (flightImages.length === 0) {
        toast.error("At least one image is mandatory for Flight Preset");
        return;
      }
      dataObj = {
        airline: flightAirline.trim() || "Flight Option",
        flightNumber: flightNumber.trim(),
        departureCity: flightDepartureCity.trim(),
        arrivalCity: flightArrivalCity.trim(),
        departureTime: flightDepartureTime.trim(),
        arrivalTime: flightArrivalTime.trim(),
        class: flightClass,
        price: Number(flightPrice),
        description: flightDescription.trim(),
        images: flightImages,
      };
    }
      ...
      <div className="bg-card p-4 rounded-xl border border-border shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex items-center gap-1 bg-muted p-1 rounded-lg self-stretch md:self-auto justify-center">
          ...
          <button
            ...
            className={`px-3 py-1.5 text-xs font-semibold rounded-md flex items-center gap-1.5 transition-colors ${
              selectedType === "flight" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
            }`}
          >
              ...
              return (
                <div key={preset._id} className="bg-card border border-border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
                  <div className="relative h-44 w-full bg-slate-900 flex items-center justify-center overflow-hidden">
                    ...
                    <span className="absolute top-3 left-3 bg-blue-500/90 text-white text-xs font-bold px-2.5 py-1 rounded-md flex items-center gap-1 shadow-sm backdrop-blur-xs">
                      <Plane className="w-3.5 h-3.5" />
                      Flight Preset
                    </span>
                    <span className="absolute top-3 right-3 bg-black/60 text-white text-xs font-medium px-2.5 py-1 rounded-md backdrop-blur-xs">
                      {preset.destination}
                    </span>
                  </div>
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                    <div>
                      <div className="flex items-center justify-between">
                        <h3 className="font-bold text-lg text-foreground truncate">{f.airline}</h3>
                        {f.flightNumber && (
                      ...
                      <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
                        <MapPin className="w-3 h-3" />
                        {f.departureCity || "Origin"} → {f.arrivalCity || preset.destination}
                      </p>
                      {f.price ? (
                        <p className="text-xs font-bold text-emerald-600 mt-1">
                          ₹{f.price.toLocaleString("en-IN")} est.
                        </p>
                      ) : null}
                      {f.description && (
                        <p className="text-xs text-muted-foreground line-clamp-2 mt-2">{f.description}</p>
                      )}
                    </div>

                    <div className="pt-3 border-t border-border flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">
                        {f.images?.length || 0} image(s)
                      </span>
                      <div className="flex items-center gap-1">
                        <Button variant="ghost" size="icon" onClick={() => openEditModal(preset)} className="h-8 w-8 text-muted-foreground hover:text-foreground">
                          <Edit2 className="w-3.5 h-3.5" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-background border border-border rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="flight">Flight Preset</option>
                  </select>
                </div>
              </div>
              ...
              {/* Dynamic Fields for Flight */}
              {formType === "flight" && (
                <div className="space-y-4 pt-2 border-t border-border">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-semibold text-foreground mb-1 block">Airline Name (Optional)</label>
                      <Input
                        placeholder="e.g. VietJet Air, Emirates, Air India"
                        value={flightAirline}
                        onChange={(e) => setFlightAirline(e.target.value)}
                      />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-foreground mb-1 block">Flight Number</label>
                      <Input
                        placeholder="e.g. VJ 1806, EK 512"
                        value={flightNumber}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-semibold text-foreground mb-1 block">Departure City</label>
                      <Input
                    ...
                    <div>
                      <label className="text-xs font-semibold text-foreground mb-1 block">Arrival City</label>
                      <Input
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="text-xs font-semibold text-foreground mb-1 block">Departure Time</label>
                      <Input
                    ...
                    <div>
                      <label className="text-xs font-semibold text-foreground mb-1 block">Arrival Time</label>
                      <Input
                        placeholder="e.g. 05:40 (+1)"
                        value={flightArrivalTime}
                        onChange={(e) => setFlightArrivalTime(e.target.value)}
                      />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-foreground mb-1 block">Flight Class</label>
                      <select
                        value={flightClass}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-semibold text-foreground mb-1 block">Est. Flight Price (₹)</label>
                      <Input
                        type="number"
                        placeholder="0"
                        value={flightPrice}
                  ...
                  <div>
                    <label className="text-xs font-semibold text-foreground mb-1 block">Flight Description / Notes</label>
                    <Textarea
                      placeholder="e.g. Includes 20kg check-in baggage, meals included..."
                  ...
                  {/* Pre-uploaded Flight Images / Airline Logos */}
                  <div>
                    <label className="text-xs font-semibold text-foreground mb-1 block">Pre-Uploaded Flight Images / Aircraft Logos * <span className="text-red-500 font-bold">(Mandatory)</span></label>
                    <div className="flex flex-wrap gap-3 mb-3">
                      {flightImages.map((img, i) => (
                        <div key={i} className="relative w-20 h-20 rounded-md overflow-hidden border border-border group">
                          <Image src={img.url} alt="Flight" fill className="object-cover" />
                          <button
                            type="button"
                            onClick={() => setFlightImages((prev) => prev.filter((_, idx) => idx !== i))}
                            className="absolute top-1 right-1 bg-destructive text-white rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity"
                          >
                            <X className="w-3 h-3" />
                          </button>
                    ...
                    <label className="flex items-center justify-center gap-2 border-2 border-dashed border-border hover:border-primary/50 p-4 rounded-lg cursor-pointer bg-muted/30 transition-colors">
                      <Upload className="w-5 h-5 text-muted-foreground" />
                      <span className="text-xs font-medium text-foreground">
                        {uploading ? "Uploading images to CDN..." : "Upload Flight Images / Logos"}
                      </span>
                      <input
                        type="file"
                        multiple
                        accept="image/*"
                        disabled={uploading}
                        onChange={(e) => handleImageUpload(e, "flight")}
                        className="hidden"
                      />
                    </label>
                  </div>
