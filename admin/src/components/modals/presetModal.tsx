...
import { Preset, PresetType, PresetHotelData, PresetSightseeingData, PresetItineraryData, PresetFlightData } from "@/types/preset";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Search, Hotel, Camera, MapPin, Star, Clock, Check, X, Bookmark, Building2, Plane } from "lucide-react";
...
export default function PresetModal({
  ...
  const getTitle = () => {
    switch (targetType) {
      ...
      case "flight":
        return "Import Flight from Presets";
    }
  };
          ...
          {loading ? (
            ...
            presets.map((preset) => {
              ...
              if (targetType === "flight") {
                const flight = preset.data as PresetFlightData;
                const coverImage = flight.images?.[0]?.url;
                ...
                return (
                  ...
                  >
                    ...
                    {coverImage ? (
                      <div className="relative w-20 h-20 rounded-md overflow-hidden bg-muted flex-shrink-0">
                        <Image src={coverImage} alt={flight.airline} fill className="object-cover" />
                      </div>
                    ) : (
                      <div className="w-20 h-20 rounded-md bg-blue-50 text-blue-600 flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h4 className="font-semibold text-foreground truncate">{flight.airline}</h4>
                        {flight.flightNumber && (
                          <span className="bg-blue-500/10 text-blue-600 text-xs font-semibold px-2 py-0.5 rounded">
                            {flight.flightNumber}
                          </span>
                        )}
                      </div>
                      <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
                        <MapPin className="w-3 h-3" />
                        {flight.departureCity || "Origin"} → {flight.arrivalCity || preset.destination}
                      </p>
                      {flight.price ? (
                        <p className="text-xs font-bold text-emerald-600 mt-1">₹{flight.price.toLocaleString("en-IN")}</p>
                      ) : null}
                      {flight.description && (
                        <p className="text-xs text-muted-foreground line-clamp-2 mt-1">{flight.description}</p>
                      )}
                    </div>
                  </div>
                );
