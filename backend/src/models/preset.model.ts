...
export type PresetType = "hotel" | "sightseeing" | "itinerary" | "flight";
...
export interface IPresetFlightData {
	airline: string;
	flightNumber?: string;
...
export interface IPreset extends Document {
	...
	city?: string;
	type: PresetType;
	data: IPresetHotelData | IPresetSightseeingData | IPresetItineraryData | IPresetFlightData;
	createdAt: Date;
	updatedAt: Date;
}
...
const presetSchema = new Schema<IPreset>(
	{
		...
		type: {
			type: String,
			enum: ["hotel", "sightseeing", "itinerary", "flight"],
			required: true,
			index: true,
		},
		data: {
